...
- ✓ Review `SUMMARY.md` for any sensitive customer or business information
...
📄 **Check `SUMMARY.md` at the project root for your complete project status**
...
## Basic Workflow
 ...
 - Send `/updateSummary` to the AI chat to update the AI documentation and the root SUMMARY.md file based on the full context of all processed emails.

**Result:** SUMMARY.md at the project root contains a human readable summary of the project so far. Documentation in `aiDocs` is updated based on the full context of the emails as well.
...
- Generate a comprehensive project summary at `SUMMARY.md`

**After completion, check `SUMMARY.md` for your project status overview**
...
- Create/update `SUMMARY.md` at the project root with:
...
│   └── mcp.json           # MCP server configuration (generated by .template/scripts/init.sh)
...
- **`SUMMARY.md`** (root): Generated periodically for human stakeholders
